...
function segmentacion(){
    var i,j,k,r,g,b,dist,grosor=1;
    var sumatoriaR,sumatoriaG,sumatoriaB;
    var promedioR,promedioG,promedioB;

    //var conjuntoPxls=[64,580,29,66,570,33,68,590,37,69,660,46,73,600,55];
...
//Se ejecuta con cada cambio del slider
function tipoIMG(value){
    umbral = parseInt(value,10);
    segmentacion();
}

function seleccionPxls(){
    var i,j,r,g,b;
    var grosor = 1;
    var image2 = new Image();
    image2 = ctx1.getImageData(0,0,canvas1.width,canvas1.height);
...
function manejadorClickRaton(e){
    var relativeX = e.clientX-(canvas1.offsetLeft-Math.floor(window.scrollX));
    var relativeY = e.clientY-(canvas1.offsetTop-Math.floor(window.scrollY));
